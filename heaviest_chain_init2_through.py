...
def compute_height_lengths(xs, y_rank, M, n):
    ...
    We sweep x in decreasing order and use reversed y ranks so that
    y_next > y_cur becomes a prefix query in the reversed space.
    Runtime: O(n log M)
    """
...
def solve(points,max_iter=10000,tol=1e-6):
    ...
    # Init #2 (Depth+Height through-length):
